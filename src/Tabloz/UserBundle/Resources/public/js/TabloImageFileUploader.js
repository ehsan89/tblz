function PunkAveTabloImageFileUploader(options)
{
	...
	self.errorCallback = 'errorCallback' in options ? options.errorCallback : function( info ) { if (window.console && console.log) { console.log(info) } },
		...
		_.each(files, function(file) {
			...
			editor.fileupload({
				...
				process: [
				          ...
				          },
				          {
				        	  ...
				        	  $el.find('.uploader_progressbar').width('100%');
				        	  $el.find('.uploader_progress_percentage').text('100%');
				        	  ...
				        	  if (data)
				        	  {
				        		  ...
				        		  $('#tablo_form_container input, #tablo_form_container textarea, #tablo_form_container select').removeClass('disabled');
				        		  $('#tablo_form_container input, #tablo_form_container textarea, #tablo_form_container select').removeAttr('readonly');
				        		  $('#tablo_form_container').show(500);
				        	  }

				          },
				          start: function (e) {
				        	  $el.find('.uploader_progressbar').width('0%');
				        	  $el.find('.uploader_progress_percentage').text('0%');
				          ...
				          progress: function (e, data) {
				        	  ...
				        	  $el.find('.uploader_progressbar').width(progress+'%');
				        	  $el.find('.uploader_progress_percentage').text(progress+'%');
				          },
				          stop: function (e) {
			...
			editor.bind('fileuploadprogress', function (e, data) {console.log('progress: ' + data);});
